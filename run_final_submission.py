...
def run_period(month_num, label, month_start, month_end):
    """
    Train on ALL data before the target month. The last 4 weeks of
    training are used as an early-stopping signal only — optimal rounds
    are found there, then the final model is retrained on all data for
    that many rounds. No data is withheld from the final model.
    ...
    # Last 4 weeks of training used only for early stopping — not held out from training data
    es_split  = train_end - pd.Timedelta(days=28)
    ...
    print(f"  ES val: {df_val_es.index[0].date()} – {df_val_es.index[-1].date()}  "
          f"({len(df_val_es):,} rows, early-stop only)")
    print(f"  Test  : {df_test.index[0].date()} – {df_test.index[-1].date()}  "
          f"({len(df_test):,} rows)")

    # ── train Huber + Quantile blend (early stopping on last-4-week holdout) ──
    df_tr_log    = log_df(df_train)
    df_tr_es_log = log_df(df_tr_es)
    df_va_buf    = log_df(pd.concat([df_tr_es.iloc[-max(ENHANCED_LAGS):], df_val_es]))

    X_tr1, _ = make_Xy(df_tr_log, df_train, 1)
    ...
    for h in SPARSE_HORIZONS:
        # Early-stop split: find optimal rounds
        X_es, y_es = make_Xy(df_tr_es_log, df_tr_es, h, feat_cols)
        X_va, y_va = make_Xy(df_va_buf,    df_val_es, h, feat_cols)
        ...
        best_rounds = m_es.best_iteration

        # Final model: all training data, fixed to best_rounds found above
        X_tr, y_tr = make_Xy(df_tr_log, df_train, h, feat_cols)
    ...
    # ── generate forecasts for the test month ─────────────────────────────────
    ...
    pred_hub = {}; pred_qnt = {}
    for h in SPARSE_HORIZONS:
        X, _ = make_Xy(df_te_buf, df_test, h, feat_cols)
    ...
    # h=1 column only (interpolated across sparse horizons)
    ...
    # h=1 at t predicts load at t+1: align actuals
