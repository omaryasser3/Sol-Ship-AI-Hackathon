...
def run_mpc(df_actual, forecasts, horizon, soc_init=SOC_INIT, verbose_every=2880):
    ...
    for t in range(n):
        ...
        # Both controller and oracle use the same causal PV and sell forecasts (lag-96).
        # This isolates the cost of LOAD forecast error in the oracle gap.
        # Oracle only swaps load_hat → actual load.
        pv_h   = pv_fc  [t:t + actual_horizon_len]   # causal: yesterday's PV
        sell_h = sell_fc[t:t + actual_horizon_len]   # causal: yesterday's sell price
        buy_h  = buy_arr[t:t + actual_horizon_len]   # deterministic ToU — always known

        if forecast_arr is not None:
            load_hat = forecast_arr[t, :actual_horizon_len]   # ML load forecast
        else:
            load_hat = load_actual[t:t + actual_horizon_len]  # oracle: perfect load
