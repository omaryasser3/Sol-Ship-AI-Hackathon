...
def run_mpc(df_actual, forecasts, horizon, soc_init=SOC_INIT,
            verbose_every=2880, use_actual_pv=True):
    """
    ...
    df_actual  : DataFrame with columns load_kw, pv_kw, buy_price, sell_price,
                 indexed by 15-min timestamps. This is the GROUND TRUTH.
    forecasts  : DataFrame from forecaster.predict_multi_horizon — shape (n, horizon),
                 each row is the forecast made AT that timestep for h=1..H.
                 Must have same index as df_actual (subset OK if shorter).
                 If None, uses actual load (= ORACLE — perfect-foresight benchmark).
    horizon    : int, MPC horizon length (≤ forecasts.shape[1])
    soc_init   : initial SoC at start of run
    verbose_every : print progress every N steps
    use_actual_pv : if True (recommended), use actual PV in the horizon as if perfectly
                    forecasted. Standard in MPC literature when load forecast is what's evaluated.
    ...
    for t in range(n):
        ...
        # Load forecast for [t+1, ..., t+H]
        if forecast_arr is not None:
            load_hat = forecast_arr[t, :actual_horizon_len]   # forecast made at t for h=1..H
        else:
            # Oracle: use actual load
            load_hat = load_actual[t:t + actual_horizon_len]

        if use_actual_pv:
            pv_h = pv_actual[t:t + actual_horizon_len]
        else:
            pv_h = pv_actual[t:t + actual_horizon_len]   # could substitute pv_lag_96 etc

        buy_h  = buy_arr [t:t + actual_horizon_len]
        ...
        p_ch_actual  = sol["p_ch"]
        p_dis_actual = sol["p_dis"]
        ...
        # Update SoC physically using ACTUAL battery commands
        if p_ch_actual > 0:
            de = p_ch_actual * ETA_DIR * DT_HOURS
        else:
            de = -p_dis_actual / ETA_DIR * DT_HOURS
        ...
        rows.append({
            ...
            "buy_price":   float(buy_h[0]),
            "sell_price":  float(sell_h[0]),
            "p_ch":        p_ch_actual,
            "p_dis":       p_dis_actual,
