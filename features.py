...
def build_features(df, lags=None, roll_windows=None, horizon=1,
    ...
    # holiday adjacency: distance in days to nearest past / future holiday
    if len(holiday_set) > 0:
        hol_d = np.array(sorted(holiday_set), dtype="datetime64[D]")
        ts_d  = ts.normalize().values.astype("datetime64[D]")
        idx_next = np.searchsorted(hol_d, ts_d, side="left")
        idx_prev = idx_next - 1
        # clamp end-cases to bracket value (90d sentinel keeps gradient bounded)
        days_next = np.where(
            idx_next < len(hol_d),
            (hol_d[np.clip(idx_next, 0, len(hol_d) - 1)] - ts_d).astype("timedelta64[D]").astype(int),
            90,
        )
        days_prev = np.where(
            idx_prev >= 0,
            (ts_d - hol_d[np.clip(idx_prev, 0, len(hol_d) - 1)]).astype("timedelta64[D]").astype(int),
            90,
        )
        out["days_to_next_holiday"] = np.clip(days_next, 0, 90)
        out["days_since_holiday"]   = np.clip(days_prev, 0, 90)
    else:
        out["days_to_next_holiday"] = 90
        out["days_since_holiday"]   = 90
    ...
    # robust rolling stats — median is spike-resistant, IQR captures dispersion
    out["load_roll_median_96"]  = load_s1.rolling(96,  min_periods=1).median()
    out["load_roll_median_672"] = load_s1.rolling(672, min_periods=1).median()
    q75_96 = load_s1.rolling(96, min_periods=4).quantile(0.75)
    q25_96 = load_s1.rolling(96, min_periods=4).quantile(0.25)
    out["load_roll_iqr_96"]  = q75_96 - q25_96
    ...
    # signed deviation in kW — complements the ratio; LGB splits more cleanly on it
    out["load_dev_from_slot_month"] = (load_s1 - sm_mean_s).values
    ...
    # steps since last spike (load_s1 > slot_month_mean + 2*std), clipped to 96
    spike_arr   = (load_s1.values > (sm_mean_s.values + 2.0 * sm_std_safe.values)).astype(np.int32)
    idx_arr     = np.arange(len(spike_arr), dtype=np.int64)
    last_spike  = np.where(spike_arr == 1, idx_arr, -1)
    last_spike  = np.maximum.accumulate(last_spike)
    steps_since = np.where(last_spike < 0, 96, idx_arr - last_spike)
    out["steps_since_spike"] = np.clip(steps_since, 0, 96).astype(np.int32)
    ...
    if include_pv and "pv_kw" in df.columns:
        ...
        # 24h rolling mean of (causal) pv_vs_expected — cloud-cover persistence proxy
        pv_vs_exp_s1 = pv_s1 / (solar_proxy + 0.01)
        out["pv_vs_expected_roll_96"] = pv_vs_exp_s1.rolling(96, min_periods=4).mean()
